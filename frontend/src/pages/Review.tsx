...
export default function Review() {
  ...
  const load = async () => {
    ...
    try {
      const r = await api.get(
        `/decks/${deckId}/queue${ahead > 0 ? `?ahead=${ahead}` : ""}`
      );
      setQueue(r.data);
      localStorage.setItem(deckKey, JSON.stringify(r.data));
    } catch (e) {
  ...
  const answer = async (grade: number) => {
    ...
    try {
      await api.post(`/cards/${current._id}/review`, { grade });
    } finally {
      setBusy(false);
    }
  };
    ...
    return (
      <div className="mt-12 max-w-lg">
        <div className="text-lg font-medium">
          No cards {ahead > 0 ? "available" : "due"}.
        </div>
  ...
  const pct = Math.round(
    ((Math.max(remaining, 1) - 1) / Math.max(remaining, 1)) * 100
  );
    ...
    <div className="space-y-4">
      ...
      {!showBack ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          ...
          <Button variant="outline" aria-label="Hard" onClick={() => answer(1)}>
            Hard (2)
          </Button>
          <Button
            variant="secondary"
          ...
          <Button aria-label="Easy" onClick={() => answer(3)}>
            Easy (4)
          </Button>
        </div>
      )}
